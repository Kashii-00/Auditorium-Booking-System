...
router.get('/', (req, res) => {
  db.query(
    `SELECT l.*, c.courseName, u.name as userName
     FROM lecturers l
...
router.post(
  ...
  upload.fields([
    ...
    { name: 'education_certificate_file' }, { name: 'cdc_book_file' },
    { name: 'driving_trainer_license_file' }, { name: 'other_documents_file' }
  ]),
  (req, res) => {
    const {
    ...
    db.query(
      ...
      [
        user_id, course_id, category, grade, stream, module,
        files.nic_file?.[0]?.filename || null,
        files.photo_file?.[0]?.filename || null,
        files.passbook_file?.[0]?.filename || null,
        files.education_certificate_file?.[0]?.filename || null,
        files.cdc_book_file?.[0]?.filename || null,
        files.driving_trainer_license_file?.[0]?.filename || null,
        files.other_documents_file?.[0]?.filename || null
      ],
      (err, result) => {
        if (err) {
          return res.status(500).json({ error: 'DB error', details: err });
        }
        res.json({ success: true, lecturerId: result.insertId });
      }
    );
  }
);
