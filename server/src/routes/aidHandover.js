...
router.post("/", auth.authMiddleware, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      logger.info(
        `Aid handover created successfully at: ${logTime} by user: ${user_name}`
      );
      return res.json({
        success: true,
  ...
  db.query(checkSql, [request_id], (checkErr, checkResults) => {
    ...
    if (checkResults.length === 0) {
      ...
      db.query(
        ...
        [
          request_id,
          items_taken_over || null,
          items_returned || null,
          receiver_name || null,
          receiver_designation || null,
          receiver_date || null,
          handover_confirmer_name || null,
          handover_confirmer_designation || null,
          handover_confirmer_date || null,
        ],
        (insertErr, insertResult) => {
          ...
          logger.info(
            `Aid handover created successfully at ${moment().format(
              "YYYY-MM-DD HH:mm:ss"
            )} by user: ${user_name}`
          );
          return res.json({
      ...
      db.query(updateSql, values, (updateErr, updateResult) => {
        ...
        logger.info(
          `Aid handover updated successfully at ${moment().format(
            "YYYY-MM-DD HH:mm:ss"
          )} by user: ${user_name}`
        );
        return res.json({
...
router.get("/request/:requestId", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, [requestId], (err, results) => {
    if (err) {
      logger.error(
        `Error fetching aid handover for request ID ${requestId}:`,
        err
      );
      return res.status(500).json({ error: "Database error" });
    }
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ error: "No aid handover found for the given request ID" });
    }
...
router.delete("/request/:requestId", auth.authMiddleware, (req, res) => {
  const now = new Date().toISOString();
  console.log(
    `[${now}] DELETE /api/aidhandover/request/${req.params.requestId}`
  );
