...
router.get("/:batchId/students", auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(
      `SELECT s.id, s.full_name, s.email, s.id_number, s.emergency_contact_number, sb.status, sb.student_code, sb.created_at as enrollment_date
...
router.get("/:batchId/lecturers", auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(
      `SELECT l.id, l.full_name, l.email, l.phone, l.category, lb.module, lb.hours_assigned, lb.status, lb.created_at
...
router.post("/:batchId/lecturers", auth.authMiddleware, async (req, res) => {
  try {
    let { lecturer_ids, lecturers } = req.body

    // Handle both old format (lecturer_ids) and new format (lecturers with details)
    if (lecturers && Array.isArray(lecturers)) {
      // New format with module and hours
      if (lecturers.length === 0) {
        return res.status(400).json({ error: "No lecturers selected" })
      }

      // Validate lecturer data
      const validLecturers = lecturers.filter(lecturer => 
        lecturer.lecturer_id && 
        !isNaN(Number.parseInt(lecturer.lecturer_id)) &&
        lecturer.module &&
        lecturer.module.trim() &&
        lecturer.hours_assigned &&
        !isNaN(Number.parseInt(lecturer.hours_assigned))
      )

      if (validLecturers.length === 0) {
        return res.status(400).json({ error: "Invalid lecturer data provided" })
      }
      ...
      // Insert lecturer-batch assignments with module and hours
      let successCount = 0
      for (const lecturer of validLecturers) {
        try {
          const result = await db.queryPromise(
            `INSERT IGNORE INTO lecturer_batches (lecturer_id, batch_id, module, hours_assigned, status, created_at) 
             VALUES (?, ?, ?, ?, 'Assigned', NOW())`,
            [lecturer.lecturer_id, req.params.batchId, lecturer.module.trim(), lecturer.hours_assigned],
          )
          if (result.affectedRows > 0) {
            successCount++
          }
        } catch (err) {
          console.error(`Error inserting lecturer ${lecturer.lecturer_id} into batch:`, err)
        }
      }
...
// PUT update lecturer details in batch
router.put("/:batchId/lecturers/:lecturerId", auth.authMiddleware, async (req, res) => {
  try {
    const { batchId, lecturerId } = req.params
    const { module, hours_assigned } = req.body
    ...
    const hoursAssigned = parseInt(hours_assigned) || 0
    if (hoursAssigned < 0 || hoursAssigned > 200) {
      return res.status(400).json({ error: "Hours assigned must be between 0 and 200" })
    }

    // Check if lecturer is assigned to the batch
    const [existingAssignment] = await db.queryPromise(
      "SELECT id FROM lecturer_batches WHERE batch_id = ? AND lecturer_id = ?",
      [batchId, lecturerId]
    )
    ...
    const result = await db.queryPromise(
      `UPDATE lecturer_batches 
       SET module = ?, hours_assigned = ?, updated_at = NOW()
       WHERE batch_id = ? AND lecturer_id = ?`,
      [module.trim(), hoursAssigned, batchId, lecturerId]
    )

    if (result.affectedRows === 0) {
      return res.status(404).json({ error: "Failed to update lecturer details" })
    }
    ...
    console.error("Error updating lecturer details:", err)
    res.status(500).json({ error: "Failed to update lecturer details" })
  }
})
