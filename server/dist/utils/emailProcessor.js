...
const processPendingEmails = async () => {
  try {
    ...
    for (const req of requests) {
      ...
      const formattedCancelDates = cancelDates.length ? cancelDates.map(d => `  - ${formatDate(d)}`).join("\n") : null;
      const aidItemsList = aidItems.length ? aidItems.map(item => {
        const base = `  ${item.item_no}. ${item.description} - Qty: ${item.quantity}${item.remark ? ` (${item.remark})` : ""}`;
        const isAuditoriumOrMisc = (item.description || "").toLowerCase().includes("auditorium") || item.item_no === "14" || item.item_no === 14;
        const isCTMRequired = req.paid_course_or_not === "Yes" && (item.item_no === "03" || item.item_no === 3);
        const mdInfo = isAuditoriumOrMisc ? `\n      - MD Approval Required: ${item.md_approval_required_or_not || "N/A"}\n      - MD Approval Obtained: ${item.md_approval_obtained || "N/A"}\n      - MD Approval Details: ${item.md_approval_details || "N/A"}` : "";
        const ctmInfo = isCTMRequired ? `\n      - CTM Approval Obtained: ${item.CTM_approval_obtained || "N/A"}\n      - CTM Details: ${item.CTM_Details || "N/A"}` : "";
        return `${base}${mdInfo}${ctmInfo}`;
      }).join("\n") : "  None";
      const subject = status === "approved" ? "Classroom Booking Request Approved" : "Classroom Booking Request Denied";
      const body = `
...
Admin Team
      ...
      try {
        ...
        const successQuery = `
          ...
          VALUES (?, ?, ?, ?, ?, 'success', NULL)
        `;
        db.query(successQuery, [req.id, email_type, req.requesting_officer_email, subject, body]);
        ...
        const failQuery = `
          ...
          VALUES (?, ?, ?, ?, ?, 'failed', ?)
        `;
        db.query(failQuery, [req.id, email_type, req.requesting_officer_email, subject, body, error.message]);
