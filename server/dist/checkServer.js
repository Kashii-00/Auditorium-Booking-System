...
try {
  ...
  for (const file of files) {
    if (file.endsWith('.js')) {
      ...
      if (routeContent.includes('router.post(') || routeContent.includes('router.get(') || routeContent.includes('router.put(') || routeContent.includes('router.delete(')) {
...
const requiredPackages = ['@babel/cli', '@babel/core', '@babel/preset-env', 'nodemon'];
try {
  ...
  const missingPackages = requiredPackages.filter(pkg => !installedPackages[pkg] && !installedDevPackages[pkg]);
