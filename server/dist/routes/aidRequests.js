...
router.post("/", auth.authMiddleware, (req, res) => {
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`;
  const requestValues = [aidRequest.requesting_officer_name, aidRequest.designation, aidRequest.requesting_officer_email, aidRequest.course_name, aidRequest.duration, aidRequest.audience_type, aidRequest.no_of_participants || 0, aidRequest.course_coordinator, aidRequest.date_from, aidRequest.date_to, aidRequest.time_from, aidRequest.time_to, aidRequest.preferred_days_of_week, aidRequest.paid_course_or_not || "No", aidRequest.payment_status || "Not Set", aidRequest.signed_date, aidRequest.request_status || "pending", aidRequest.classrooms_allocated || null, aidRequest.exam_or_not || "No", aidRequest.cancelled_by_requester || "No"];
  db.query(requestSql, requestValues, (err, result) => {
    if (err) {
      logger.error("Error creating aid request:", err);
      return res.status(500).json({
        error: "Database error while inserting aid request"
      });
    }
  ...
  `;
    const itemValues = aidItems.map(item => [requestId, item.item_no, item.description, item.quantity, item.remark, item.md_approval_required_or_not || "No", item.md_approval_obtained || "No", item.md_approval_details || null, item.CTM_approval_obtained || null, item.CTM_Details || null]);
    db.query(itemSql, [itemValues], err => {
      if (err) {
        logger.error("Error inserting aid items:", err);
        return res.status(500).json({
          error: "Database error while inserting aid items"
        });
      }
...
`;
  db.query(sql, [requestId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({
        error: "Database error while fetching aid request"
      });
    }
    if (results.length === 0) {
      return res.status(404).json({
        error: "Aid request not found"
      });
    }
    const grouped = {
      ...results[0],
...
router.get("/approved/:requestId", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, [requestId], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({
        error: "Database error while fetching approved aid request"
      });
    }
    if (results.length === 0) {
      return res.status(404).json({
        error: "Approved aid request not found"
      });
    }
    const grouped = {
      ...results[0],
...
router.put("/:id", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, values, err => {
    ...
    const logintime = moment().format("YYYY-MM-DD HH:mm:ss");
    logger.info(`Aid request ID ${req.params.id} updated at: ${logintime} by: ${user_name}`);
...
router.put("/cancel-or-update/:id", auth.authMiddleware, (req, res) => {
  const now = new Date().toISOString();
  console.log(`[${now}] PUT /api/aidrequests/cancel-or-update/${req.params.id}`);
  ...
  db.query(checkSql, [req.params.id], (checkErr, results) => {
    ...
    if (currentStatus === "Approved") {
      return res.status(403).json({
        error: "Approved requests cannot be cancelled."
      });
    }
    ...
    db.query(sql, values, err => {
      ...
      const logintime = moment().format("YYYY-MM-DD HH:mm:ss");
      logger.info(`Aid request ID ${req.params.id} (cancel/update) modified at: ${logintime} by: ${user_name}`);
      logger.info(`Updated fields: ${updates.join(", ")}`);
      return res.json({
        success: true,
        message: "Aid request updated successfully"
...
router.delete("/:id", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, [req.params.id], err => {
    if (err) {
      ...
      return res.status(500).json({
        error: "Database error while deleting aid request"
      });
    }
    const logintime = moment().format("YYYY-MM-DD HH:mm:ss");
