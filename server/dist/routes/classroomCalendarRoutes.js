...
router.post("/", auth.authMiddleware, (req, res) => {
  ...
  const calendarValues = [user_id, request_id || null, date_from, date_to, time_from, time_to, course_name, preferred_days_of_week, classes_allocated];
  db.query(calendarSql, calendarValues, (err, result) => {
    ...
    const preferredDays = preferred_days_of_week.split(",").map(d => shortToFullDayMap[d.trim().slice(0, 3).toLowerCase()]).filter(Boolean);
    const allDates = [];
    ...
    const detailsSql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const detailsValues = [calendar_id, user_id, request_id || null, course_name || null, JSON.stringify(allDates), JSON.stringify(cancelDates), time_from, time_to, classes_allocated];
    db.query(detailsSql, detailsValues, err2 => {
      if (err2) {
        logger.error("Error inserting booking details:", err2);
        return res.status(500).json({
          error: "Details insert error"
        });
      }

      // <-- Add this update query here
      if (request_id) {
        db.query("UPDATE aid_requests SET last_updated = NOW() WHERE id = ?", [request_id], updateErr => {
          if (updateErr) {
            logger.error(`Error updating last_updated on aid_requests for id ${request_id}:`, updateErr);
            // Not critical: don't fail the request because of this
          }
        });
      }
      const logtime = moment().format("YYYY-MM-DD HH:mm:ss");
      logger.info(`Booking detail created at ${logtime} for calendar ID ${calendar_id}`);
      return res.json({
        success: true,
...
router.get("/details", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    const transformedResults = results.map(row => {
      ...
      const effectiveDates = allDates.filter(date => !cancelDates.includes(date));
      return {
        ...row,
...
router.get("/details-with-request-info", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    return res.json({
      success: true,
      message: "Classroom booking details with request info fetched successfully",
      data: transformedResults
    });
  });
});
...
router.put("/details/by-request/:request_id/cancel-dates", auth.authMiddleware, (req, res) => {
  const {
    request_id
  } = req.params;
  const {
  ...
  if (!Array.isArray(cancel_dates)) {
    return res.status(400).json({
      error: "cancel_dates must be an array of dates"
    });
  }
  const updateCancelDatesSql = `
    ...
    LIMIT 1
  `;
  db.query(updateCancelDatesSql, [JSON.stringify(cancel_dates), request_id], (err, result) => {
    if (err) {
      logger.error(`Error updating cancel_dates for request_id ${request_id}:`, err);
      return res.status(500).json({
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({
        error: "No booking found for this request_id"
      });
    }
    ...
    db.query(updateAidRequestSql, [request_id], aidErr => {
      if (aidErr) {
        logger.error(`Error updating last_updated for aid_request ${request_id}:`, aidErr);
        // Not critical, still return success
        return res.json({
          success: true,
          message: "cancel_dates updated, but failed to update aid_request timestamp"
        });
      }
      return res.json({
        success: true,
        message: "cancel_dates and aid_request.last_updated updated successfully"
      });
    });
  });
});
...
router.delete("/:id", auth.authMiddleware, (req, res) => {
  ...
  db.query(sql, [req.params.id], err => {
    ...
    const logtime = moment().format("YYYY-MM-DD HH:mm:ss");
    logger.info(`Calendar entry deleted at ${logtime} by user ${req.user.name}`);
    return res.json({
      success: true,
