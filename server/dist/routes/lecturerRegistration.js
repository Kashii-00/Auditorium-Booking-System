...
const storage = multer.diskStorage({
  destination(req, file, cb) {
    const folder = req.body.email ? path.join(uploadDir, req.body.email.replace(/[^a-zA-Z0-9]/g, '_')) : uploadDir;
    fs.mkdirSync(folder, {
      recursive: true
    });
    cb(null, folder);
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      return res.status(400).json({
        error: "File too large. Maximum allowed is 10MB per file."
      });
    }
    return res.status(400).json({
...
// PUT /lecturers/:id
router.put('/:id', auth.authMiddleware, (req, res, next) => {
  upload.fields([{
    name: 'nic_file'
  }, {
    name: 'photo_file'
  ...
  try {
    ...
    const [currentAcademicDetails] = await conn.queryPromise('SELECT education_certificate_file FROM lecturer_academic_details WHERE lecturer_id = ?', [req.params.id]);
...
router.get('/:id', auth.authMiddleware, async (req, res) => {
  try {
    ...
    const [lecturer] = await db.queryPromise(`SELECT l.* 
      FROM lecturers l
    ...
    res.status(500).json({
      error: 'Failed to retrieve lecturer data',
      details: err.message
    });
  }
});
...
router.delete('/:id', auth.authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await conn.queryPromise(`DELETE FROM lecturers WHERE id = ?`, [req.params.id]);
    await conn.commitPromise();
    res.json({
      success: true
    });
