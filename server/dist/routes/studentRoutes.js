...
const storage = multer.diskStorage({
  destination(req, file, cb) {
    const folder = req.body.email ? path.join(baseUploadDir, req.body.email.replace(/[^a-zA-Z0-9]/g, '_')) : baseUploadDir;
    fs.mkdirSync(folder, {
      recursive: true
    });
    cb(null, folder);
...
function parseStudent(row) {
  if (row.driving_details) {
    try {
      row.driving_details = JSON.parse(row.driving_details);
    } catch {}
  }
  return row;
}
...
router.post('/', auth.authMiddleware, uploadFields, async (req, res) => {
  ...
  try {
    ...
    if (!courseIds.length) return res.status(400).json({
    ...
    await conn.queryPromise(`INSERT INTO student_users (student_id, email, password, is_temp_password, status)
       VALUES (?, ?, ?, TRUE, 'ACTIVE')`, [insertId, data.email, hashedPassword]);
...
router.get('/:id', auth.authMiddleware, async (req, res) => {
  try {
    ...
    // courses
    student.courses = await db.queryPromise(`SELECT c.id,c.courseId,c.courseName,c.stream,sc.enrollment_date,sc.primary_course,sc.status
...
router.put('/:id', auth.authMiddleware, uploadFields, async (req, res) => {
  ...
  try {
    ...
    if (files.nic_document) {
      fields.push('nic_document_path=?');
      vals.push(files.nic_document[0].path);
    }
    if (files.passport_document) {
      fields.push('passport_document_path=?');
      vals.push(files.passport_document[0].path);
    }
    if (files.photo) {
      fields.push('photo_path=?');
      vals.push(files.photo[0].path);
    }
    if (fields.length) {
    ...
    const currentEnrollments = await conn.queryPromise('SELECT course_id, primary_course FROM student_courses WHERE student_id=?', [req.params.id]);
    ...
    if (removedCourseIds.length) {
      await conn.queryPromise('DELETE FROM student_courses WHERE student_id=? AND course_id IN (?)', [req.params.id, removedCourseIds]);
    }
    ...
    for (let i = 0; i < newCourseIds.length; i++) {
      ...
      await conn.queryPromise(`INSERT INTO student_courses (student_id, course_id, enrollment_date, primary_course, status)
         VALUES (?, ?, CURDATE(), ?, ?)`, [req.params.id, newCourseIds[i], isPrimary ? 1 : 0, 'Active']);
    }

    // Update primary course if specified
    if (data.primary_course_id) {
      // First, set all to non-primary
      await conn.queryPromise('UPDATE student_courses SET primary_course=0 WHERE student_id=?', [req.params.id]);

      // Then set the specified one as primary
      await conn.queryPromise('UPDATE student_courses SET primary_course=1 WHERE student_id=? AND course_id=?', [req.params.id, data.primary_course_id]);
    }
    await conn.commitPromise();
    res.json({
      success: true
...
router.delete('/:id', auth.authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (del.affectedRows === 0) return res.status(404).json({
      error: 'Not found'
    });
    await conn.commitPromise();
    res.json({
