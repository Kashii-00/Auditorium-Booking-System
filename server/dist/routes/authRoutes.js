...
router.post('/login', (req, res) => {
  ...
  db.query(sql, [email], async (err, results) => {
    ...
    const loginUser = () => {
      ...
      db.query(`REPLACE INTO user_refresh_tokens (user_id, refresh_token) VALUES (?, ?)`, [user.id, refreshToken], err => {
        if (err) {
...
router.post('/refresh', (req, res) => {
  try {
    ...
    db.query(`SELECT refresh_token FROM user_refresh_tokens WHERE user_id = ?`, [userData.id], (err, results) => {
      if (err) return res.status(500).json({
        error: 'Server error'
      });
...
router.post('/logout', (req, res) => {
  ...
  if (refreshToken) {
    ...
    if (userId) {
      db.query(`DELETE FROM user_refresh_tokens WHERE user_id = ?`, [userId], () => {});
    }
  }
  res.clearCookie('refreshToken', {
    httpOnly: true,
