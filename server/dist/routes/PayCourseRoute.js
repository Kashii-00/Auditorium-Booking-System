...
router.post('/', auth.authMiddleware, async (req, res) => {
  try {
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const params = [student_id, course_id, payment_type, amount, payment_date, payment_method, receipt_number, notes, req.user.id];
...
router.put('/:id', auth.authMiddleware, async (req, res) => {
  try {
    ...
    const sql = `
      ...
      WHERE id = ?
    `;
    const params = [payment_type, amount, payment_date, payment_method, receipt_number || null, notes || null, req.params.id];
    const result = await db.queryPromise(sql, params);
