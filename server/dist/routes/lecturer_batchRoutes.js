  ...
  try {
    ...
    if (!decoded.lecturerId) {
      return res.status(403).json({
        error: 'Not authorized as lecturer'
      });
    }
...
router.get('/batch/:batchId', lecturerAuthMiddleware, async (req, res) => {
  try {
    ...
    const accessCheck = await db.queryPromise('SELECT COUNT(*) as count FROM lecturer_batches WHERE batch_id = ? AND lecturer_id = ?', [batchId, req.lecturer.lecturerId]);
...
router.delete('/materials/:materialId', lecturerAuthMiddleware, async (req, res) => {
  try {
    ...
    const material = await db.queryPromise('SELECT * FROM batch_materials WHERE id = ? AND lecturer_id = ?', [materialId, req.lecturer.lecturerId]);
    if (material.length === 0) {
      return res.status(403).json({
        error: 'Access denied to this material'
      });
    ...
    await db.queryPromise('DELETE FROM batch_materials WHERE id = ?', [materialId]);
    res.json({
      success: true,
...
router.put('/assignments/:assignmentId', lecturerAuthMiddleware, async (req, res) => {
  try {
    ...
    const assignment = await db.queryPromise('SELECT * FROM assignments WHERE id = ? AND lecturer_id = ?', [assignmentId, req.lecturer.lecturerId]);
...
router.put('/announcements/:announcementId', lecturerAuthMiddleware, async (req, res) => {
  try {
    ...
    const announcement = await db.queryPromise('SELECT * FROM announcements WHERE id = ? AND lecturer_id = ?', [announcementId, req.lecturer.lecturerId]);
...
router.get('/assignments/:assignmentId/stats', lecturerAuthMiddleware, async (req, res) => {
  try {
    ...
    result.submission_rate = result.total_students > 0 ? (result.total_submissions / result.total_students * 100).toFixed(1) : '0.0';
    result.grading_progress = result.total_submissions > 0 ? (result.graded_submissions / result.total_submissions * 100).toFixed(1) : '0.0';
    res.json({
      assignment: {
        id: assignment.id,
        title: assignment.title,
