...
router.get('/', auth.authMiddleware, async (req, res) => {
  try {
    ...
    if (batchIds.length > 0) {
      // Get student counts
      const studentRows = await db.queryPromise(`SELECT batch_id, COUNT(*) as count FROM student_batches WHERE batch_id IN (?) GROUP BY batch_id`, [batchIds]);
      studentRows.forEach(row => {
        studentCounts[row.batch_id] = row.count;
      });
      // Get lecturer counts
      const lecturerRows = await db.queryPromise(`SELECT batch_id, COUNT(*) as count FROM lecturer_batches WHERE batch_id IN (?) GROUP BY batch_id`, [batchIds]);
...
router.get('/annual-plan', auth.authMiddleware, async (req, res) => {
  ...
  if (!course_id || !year) {
    return res.status(400).json({
      error: 'Course ID and year are required'
    });
  }
  ...
  `;
  try {
    const plans = await db.queryPromise(sql, [course_id, startDate, endDate, startDate, endDate, startDate, endDate]);
    res.json(plans);
  } catch (err) {
...
router.get('/:id', auth.authMiddleware, async (req, res) => {
  try {
    const [batch] = await db.queryPromise(`SELECT b.*, 
        (SELECT COUNT(*) FROM student_batches sb WHERE sb.batch_id = b.id) AS student_count,
        (SELECT COUNT(*) FROM lecturer_batches lb WHERE lb.batch_id = b.id) AS lecturer_count
...
router.get('/:batchId/lecturers', auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(`SELECT l.id, l.full_name, l.email, l.phone
...
router.get('/:batchId/students', auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(`SELECT s.id, s.full_name, s.email
...
router.post('/:batchId/lecturers', auth.authMiddleware, async (req, res) => {
  try {
    ...
    const [batch] = await db.queryPromise(`SELECT capacity, (SELECT COUNT(*) FROM lecturer_batches WHERE batch_id = ?) AS lecturer_count FROM batches WHERE id = ?`, [req.params.batchId, req.params.batchId]);
    if (!batch) return res.status(404).json({
      error: 'Batch not found'
    });
    if (batch.lecturer_count + lecturer_ids.length > batch.capacity) {
      return res.status(400).json({
        error: 'Batch capacity reached'
      });
    }
    // Insert lecturer-batch assignments (ignore duplicates)
    for (const lecturerId of lecturer_ids) {
      await db.queryPromise(`INSERT IGNORE INTO lecturer_batches (lecturer_id, batch_id, status) VALUES (?, ?, 'Assigned')`, [lecturerId, req.params.batchId]);
    }
    res.json({
      success: true
    });
...
router.post('/:batchId/students', auth.authMiddleware, async (req, res) => {
  try {
    ...
    // Check batch capacity
    const [batch] = await db.queryPromise(`SELECT capacity, (SELECT COUNT(*) FROM student_batches WHERE batch_id = ?) AS student_count FROM batches WHERE id = ?`, [req.params.batchId, req.params.batchId]);
    if (!batch) return res.status(404).json({
      error: 'Batch not found'
    });
    ...
    for (const studentId of student_ids) {
      await db.queryPromise(`INSERT IGNORE INTO student_batches (student_id, batch_id, status) VALUES (?, ?, 'Active')`, [studentId, req.params.batchId]);
    }
    res.json({
      success: true
    });
