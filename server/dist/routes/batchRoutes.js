...
router.get("/", auth.authMiddleware, async (req, res) => {
  try {
    ...
    if (batchIds.length > 0) {
      // Get student counts
      const studentRows = await db.queryPromise(`SELECT batch_id, COUNT(*) as count FROM student_batches WHERE batch_id IN (${batchIds.map(() => "?").join(",")}) GROUP BY batch_id`, batchIds);
      studentRows.forEach(row => {
        studentCounts[row.batch_id] = row.count;
      });

      // Get lecturer counts
      const lecturerRows = await db.queryPromise(`SELECT batch_id, COUNT(*) as count FROM lecturer_batches WHERE batch_id IN (${batchIds.map(() => "?").join(",")}) GROUP BY batch_id`, batchIds);
...
router.get("/:id", auth.authMiddleware, async (req, res) => {
  try {
    const [batch] = await db.queryPromise(`SELECT b.*, c.courseName, c.courseId,
...
router.post("/", auth.authMiddleware, async (req, res) => {
  ...
  if (!course_id || !batch_name || !start_date) {
    return res.status(400).json({
      error: "course_id, batch_name, and start_date are required"
    });
  }
  try {
...
router.put("/:id", auth.authMiddleware, async (req, res) => {
  ...
  if (!batch_name || !start_date) {
    return res.status(400).json({
      error: "batch_name and start_date are required"
    });
  }
  const sql = `
  ...
  `;
  try {
    const result = await db.queryPromise(sql, [batch_name, start_date, end_date, capacity || 30, location || "", schedule || "", id]);
...
router.delete("/:id", auth.authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [{
      count
    }] = await db.queryPromise("SELECT COUNT(*) AS count FROM student_batches WHERE batch_id = ?", [id]);
    if (count > 0) {
      return res.status(400).json({
...
router.get("/:batchId/students", auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(`SELECT s.id, s.full_name, s.email, s.id_number, s.emergency_contact_number, sb.status, sb.created_at as enrollment_date
       FROM student_batches sb
       JOIN students s ON sb.student_id = s.id
       WHERE sb.batch_id = ?
       ORDER BY s.full_name`, [req.params.batchId]);
    res.json(rows);
  } catch (err) {
    console.error("Error loading batch students:", err);
    res.status(500).json({
      error: "Failed to load batch students"
    });
  }
});
...
router.get("/:batchId/lecturers", auth.authMiddleware, async (req, res) => {
  try {
    const rows = await db.queryPromise(`SELECT l.id, l.full_name, l.email, l.phone, lb.module, lb.status
       FROM lecturer_batches lb
       JOIN lecturers l ON lb.lecturer_id = l.id
       WHERE lb.batch_id = ?
       ORDER BY l.full_name`, [req.params.batchId]);
    res.json(rows);
  } catch (err) {
    console.error("Error loading batch lecturers:", err);
    res.status(500).json({
      error: "Failed to load batch lecturers"
    });
  }
});
...
router.get("/:batchId/available-students", auth.authMiddleware, async (req, res) => {
  try {
    ...
    const availableStudents = await db.queryPromise(`SELECT DISTINCT s.id, s.full_name, s.email, s.id_number, s.emergency_contact_number
       FROM students s
       ...
       ORDER BY s.full_name`, [batch.course_id, batchId]);
...
router.post("/:batchId/students", auth.authMiddleware, async (req, res) => {
  try {
    ...
    const [batch] = await db.queryPromise(`SELECT id, capacity, course_id,
        (SELECT COUNT(*) FROM student_batches WHERE batch_id = ?) AS student_count 
       FROM batches WHERE id = ?`, [req.params.batchId, req.params.batchId]);
    if (!batch) {
      return res.status(404).json({
        error: "Batch not found"
      });
    ...
    const existingStudents = await db.queryPromise(`SELECT DISTINCT s.id FROM students s
       INNER JOIN student_courses sc ON s.id = sc.student_id
       WHERE s.id IN (${validStudentIds.map(() => "?").join(",")})
       AND sc.course_id = ?
       AND sc.status = 'Active'`, [...validStudentIds, batch.course_id]);
    ...
    for (const studentId of existingStudentIds) {
      try {
        ...
        const result = await db.queryPromise(`INSERT IGNORE INTO student_batches (student_id, batch_id, student_code, status, created_at) 
           VALUES (?, ?, ?, 'Active', NOW())`, [studentId, req.params.batchId, studentCode]);
        if (result.affectedRows > 0) {
          // Update the student_courses record with the generated student code
          await db.queryPromise(`UPDATE student_courses 
             SET student_code = ? 
             WHERE student_id = ? AND course_id = ? AND student_code IS NULL`, [studentCode, studentId, batch.course_id]);
    ...
    if (successCount === 0) {
      return res.status(400).json({
        error: "All selected students are already in this batch"
      });
    }
  ...
  try {
    ...
    if (typeof lecturer_ids === "string") {
      try {
        ...
        lecturer_ids = lecturer_ids.split(",").map(id => id.trim()).filter(id => id);
      }
    }
    if (!Array.isArray(lecturer_ids) || lecturer_ids.length === 0) {
      return res.status(400).json({
        error: "No lecturers selected"
      });
    }
    ...
    if (validLecturerIds.length === 0) {
      return res.status(400).json({
        error: "Invalid lecturer IDs provided"
      });
    }
    ...
    for (const lecturerId of validLecturerIds) {
      try {
        const result = await db.queryPromise(`INSERT IGNORE INTO lecturer_batches (lecturer_id, batch_id, status, created_at) 
           VALUES (?, ?, 'Assigned', NOW())`, [lecturerId, req.params.batchId]);
        if (result.affectedRows > 0) {
          successCount++;
        }
      } catch (err) {
    ...
    res.status(500).json({
      error: "Failed to assign lecturers to batch"
    });
  }
});
...
router.delete("/:batchId/students/:studentId", auth.authMiddleware, async (req, res) => {
  try {
    ...
    const result = await db.queryPromise(`DELETE FROM student_batches WHERE batch_id = ? AND student_id = ?`, [batchId, studentId]);
    if (result.affectedRows === 0) {
      return res.status(404).json({
...
router.delete("/:batchId/lecturers/:lecturerId", auth.authMiddleware, async (req, res) => {
  try {
    ...
    const result = await db.queryPromise(`DELETE FROM lecturer_batches WHERE batch_id = ? AND lecturer_id = ?`, [batchId, lecturerId]);
    if (result.affectedRows === 0) {
      return res.status(404).json({
        error: "Lecturer not found in batch"
      });
    }
    res.json({
      success: true
    ...
    res.status(500).json({
      error: "Failed to remove lecturer from batch"
    });
  }
});
...
router.get("/details/:batchId", auth.authMiddleware, async (req, res) => {
  try {
    ...
    const [batch] = await db.queryPromise(`SELECT b.*, c.courseName, c.courseId, c.stream, c.description as course_description,
    ...
    const students = await db.queryPromise(`SELECT s.id, s.full_name, s.email, s.id_number, s.emergency_contact_number, 
    ...
    const lecturers = await db.queryPromise(`SELECT l.id, l.full_name, l.email, l.phone, l.category,
              lb.module, lb.status, lb.hours_assigned, lb.payment_rate
       FROM lecturer_batches lb
       JOIN lecturers l ON lb.lecturer_id = l.id
       WHERE lb.batch_id = ?
       ORDER BY l.full_name`, [batchId]);
