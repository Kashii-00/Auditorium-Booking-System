...
const EditRatesPanel = () => {
  ...
  const handleUpdateRate = async (rate) => {
    ...
    try {
      await authRequest("patch", "http://localhost:5003/api/rates", {
        items: [
          {
  ...
  const confirmDelete = async (rate) => {
    try {
      await authRequest(
        "delete",
        `http://localhost:5003/api/rates/by-item?itemDescription=${encodeURIComponent(
          rate.item_description
  ...
  const handleAddRate = async () => {
    ...
    try {
      await authRequest("post", "http://localhost:5003/api/rates", {
        items: [newRate],
      });
