...
const PaymentSingleDetails = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await authRequest(
        "get",
        getApiUrl(`/payment-sf-display/${paymentMainDetailsId}`)
      );
      setData(res);
  ...
  const handleRefreshSummary = async () => {
    ...
    try {
      ...
      await authRequest(
        "delete",
        getApiUrl(`/payment-course-final-summary/${data.course_cost_summary.id}`)
      );
                  ...
                  <>
                    ...
                    <div className="pfbtns print-hidden">
                      <button
                        onClick={() =>
                          handleDelete(
                            getApiUrl(`/course-development-work/full/${data.course_development_work.main.id}`),
                            "Course Development Work"
                          )
                        }
                  ...
                  <>
                    ...
                    <div className="pfbtns print-hidden">
                      <button
                        onClick={() =>
                          handleDelete(
                            getApiUrl(`/course-delivery-cost-full/full/${data.course_delivery_costs.main.id}`),
                            "Course Delivery Costs"
                          )
                        }
                        className="delete-btn3"
                  ...
                  <>
                    ...
                    <div className="pfbtns print-hidden">
                      <button
                        onClick={() =>
                          handleDelete(
                            getApiUrl(`/course-overheads-cost/full/${data.course_overheads_main.main.id}`),
                            "Course Overheads"
                          )
                        }
                        className="delete-btn3"
