...
const BusCalendarFull = ({ user = { id: 1, name: "Demo User" } }) => {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      setLoading(true)
              const response = await authRequest("get", getApiUrl("/busBookings"))
  ...
  const handleBooking = useCallback(async () => {
    ...
    try {
      const { travelDate, returnDate, fromPlace, toPlace, forWho, ContactNo } = formState
              ...
              const response = await authRequest("post", getApiUrl("/busBookings"), {
        user_id: user.id,
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 relative">
      ...
      <div className="relative z-10 px-4 xl:px-6 pb-4 xl:pb-6 pt-6 flex flex-col lg:flex-row gap-6 min-h-[calc(100vh-200px)]">
        {/* Calendar Section */}
        <div className="flex-1 flex flex-col">
          <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-xl flex-1 flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col">
              {/* Calendar Grid */}
              <div className="grid grid-cols-7 gap-2 sm:gap-3 mb-4 flex-shrink-0">{weekdayHeaders}</div>
              <div className="grid grid-cols-7 gap-2 sm:gap-3 flex-1">{calendarDays}</div>
            </CardContent>
          </Card>
        </div>

        {/* Sidebar */}
        <div className="w-full lg:w-80 flex flex-col gap-6 min-h-[calc(100vh-200px)]">


          {/* Selected Date Reservations */}
          <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-xl flex-1 flex flex-col">
            <CardHeader className="pb-4 bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50 border-b border-slate-100 rounded-t-2xl flex-shrink-0">
          ...
          <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-xl flex-1 flex flex-col">
            <CardContent className="p-0 flex-1 flex flex-col">
              <Tabs defaultValue="upcoming" className="w-full flex-1 flex flex-col">
                <TabsList className="grid w-full grid-cols-2 bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50 rounded-t-2xl p-1 flex-shrink-0">
                  <TabsTrigger
                    value="upcoming"
                ...
                <div className="p-4 flex-1 flex flex-col">
                  <TabsContent value="upcoming" className="mt-0 flex-1 flex flex-col">
                    <div className="flex-1 flex flex-col">
                  ...
                  <TabsContent value="past" className="mt-0 flex-1 flex flex-col">
                    <div className="flex flex-col flex-1">
                      ...
                      {pastBookings.length > 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-8 text-slate-500 flex-1">
                          <CalendarIcon className="h-12 w-12 text-slate-300 mb-3" />
