...
const StatCard = memo(({ title, value, icon: Icon, color = "blue", progress = 75 }) => {
  ...
  const circumference = 2 * Math.PI * 42 // radius = 42 for smaller circles
  const strokeDasharray = circumference
  const strokeDashoffset = circumference - (progress / 100) * circumference

  return (
    <div className="flex flex-col items-center group">
      {/* Circular card with progress ring - Much smaller responsive sizes */}
      <div className="relative pt-2">
        {/* Background circle - Compact sizing for specific resolutions */}
      ...
      {/* Title - Compact responsive sizing */}
...
// Optimized Booking Row Component - Styled like Lecturer Registration
  ...
  return (
    ...
    >
      ...
      <td className="p-4">
        <div className="flex items-center gap-3">
          <Avatar className="w-12 h-12 border-2 border-blue-200">
            <AvatarImage src={`/placeholder.svg?height=48&width=48&query=organizer`} />
          ...
          <div>
            <div className="font-bold text-slate-900">{booking.name || "N/A"}</div>
      ...
      <td className={`p-4 transition-all duration-300 ${
        ...
        <div className="text-sm font-semibold text-slate-700">{booking.email || "N/A"}</div>
      </td>
      <td className={`p-4 transition-all duration-300 ${
        !sidebarCollapsed ? 'hidden opacity-0 w-0' : 'table-cell opacity-100'
      }`}>
        <div className="text-sm">
          ...
          <div className="text-slate-600 font-medium">{booking.phone || "-"}</div>
        </div>
      </td>
      <td className="p-4">
        <div className="text-sm">
          <div className="font-bold text-slate-900 max-w-[200px] truncate">{booking.description}</div>
          <div className="text-slate-600 font-medium">Auditorium Booking</div>
        </div>
      </td>
      <td className="p-4">
        <div className="text-sm">
...
// Table Header Component - Styled like Lecturer Registration
  ...
  return (
    <thead>
      <tr className="border-b-2 border-slate-200 bg-gradient-to-r from-slate-50 via-blue-50 to-indigo-50">
        ...
        <th className="text-left p-4 font-black text-slate-700">Event Details</th>
        <th
          className="text-left p-4 cursor-pointer hover:bg-blue-100 transition-colors"
...
// Pagination Controls Component - Styled like Lecturer Registration
            ...
            {Array.from({ length: Math.min(7, totalPages) }, (_, i) => {
              ...
              if (totalPages <= 7) {
                ...
                if (currentPage <= halfVisible) {
                  // Near the beginning
                  pageNumber = i + 1
                  if (pageNumber > maxVisiblePages) return null
                } else if (currentPage >= totalPages - halfVisible) {
                  // Near the end
                  pageNumber = totalPages - maxVisiblePages + i + 1
                  if (pageNumber <= 0) return null
                } else {
                  // In the middle
                  pageNumber = currentPage - halfVisible + i
...
// Records Per Page Selector Component - Styled like Lecturer Registration
...
export default function EventBookingFull() {
  ...
  const fetchBookings = useCallback(async () => {
    try {
      setLoading(true)
      const bookingsData = await authRequest("get", getApiUrl("/bookings"))
  ...
  const updateStatus = useCallback(
    async (id, status) => {
      try {
        ...
        await authRequest("put", getApiUrl(`/bookings/${id}`), { status })
  ...
  const deleteBooking = useCallback(
    async (id) => {
      ...
      try {
        setLoading(true)
        await authRequest("delete", getApiUrl(`/bookings/${id}`))
    ...
    async (action) => {
      ...
      try {
        ...
        const promises = selectedBookings.map((id) => {
          switch (action) {
            case "approve":
              return authRequest("put", getApiUrl(`/bookings/${id}`), { status: "APPROVED" })
            case "reject":
              return authRequest("put", getApiUrl(`/bookings/${id}`), { status: "DENIED" })
            case "delete":
              return authRequest("delete", getApiUrl(`/bookings/${id}`))
            default:
  ...
  const filteredBookings = useMemo(() => {
    try {
      ...
      return bookings.filter((booking) => {
        ...
        if (filterMonth !== "ALL" && booking.booking_date) {
          try {
            matchesMonth = new Date(booking.booking_date).getMonth() === Number.parseInt(filterMonth)
          } catch (dateError) {
            console.warn("Error parsing booking date:", booking.booking_date, dateError)
            matchesMonth = false
          }
        }
    ...
    try {
      ...
      return [...filteredBookings].sort((a, b) => {
        ...
        switch (sort.key) {
          ...
          case "date":
          case "booking_date":
            valueA = a.booking_date ? new Date(a.booking_date).getTime() : 0
            valueB = b.booking_date ? new Date(b.booking_date).getTime() : 0
            break
          case "status":
            valueA = a.status || ""
            valueB = b.status || ""
  ...
  const paginatedBookings = useMemo(() => {
    try {
      ...
      console.error("Error paginating bookings:", error)
      return []
    }
  ...
  const exportToCSV = useCallback(() => {
    ...
    try {
      ...
      setPopupMessage("CSV export completed successfully")
      setShowPopup(true)
      setTimeout(() => setShowPopup(false), 3000)
    } catch (error) {
      ...
      setTimeout(() => setExportLoading(false), 1000) // Small delay to show the loading state
    }
  ...
  useEffect(() => {
    if (highlightedBookingId && bookings.length > 0 && sortedBookings.length > 0) {
      try {
        // Find the booking in the list
        const highlightedBooking = bookings.find((booking) => booking && booking.id === highlightedBookingId)
        if (highlightedBooking) {
          // Calculate which page this booking should be on
          const bookingIndex = sortedBookings.findIndex((booking) => booking && booking.id === highlightedBookingId)
          if (bookingIndex !== -1 && itemsPerPage > 0) {
          ...
          console.warn("Highlighted booking not found:", highlightedBookingId)
        }
      } catch (error) {
        console.error("Error handling highlighted booking:", error)
      }
  ...
  useEffect(() => {
    const handleNavigateToBookings = (event) => {
      try {
        ...
        if (!event || !event.detail) {
          console.warn("Event or event.detail is missing")
          return
        }
        ...
        if (highlight && bookingId) {
          console.log("Setting highlighted booking ID:", bookingId)
    ...
    try {
      ...
      if (storedHighlightId) {
        console.log("Found stored highlight ID:", storedHighlightId)
  ...
  useLayoutEffect(() => {
    ...
    style.id = "event-booking-performance-css"

    // Remove existing style if present
    const existingStyle = document.getElementById("event-booking-performance-css")
    if (existingStyle) {
    ...
    return () => {
      const styleToRemove = document.getElementById("event-booking-performance-css")
      if (styleToRemove) {
  ...
  return (
    ...
    >
      ...
      {/* Bulk action loading screen */}
      {bulkActionLoading && (
        <Suspense fallback={<div>Processing...</div>}>
          <div className="fixed inset-0 z-50">
            <LoadingScreen message="Processing bulk action..." type="bookings" />
          </div>
        </Suspense>
      )}
      ...
      {exportLoading && (
        <Suspense fallback={<div>Exporting...</div>}>
          <div className="fixed inset-0 z-50">
            <LoadingScreen message="Exporting CSV data..." type="bookings" />
          </div>
        </Suspense>
      )}
        ...
        <Suspense fallback={<div>Loading...</div>}>
          <div className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white rounded-2xl p-8 shadow-2xl">
              <LoadingScreen message="Refreshing data..." type="bookings" />
            </div>
          </div>
        </Suspense>
      )}
      ...
      <div className="relative z-10 p-4 xl:p-6 space-y-6 xl:space-y-8">
        ...
        <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-xl relative z-10">
          ...
          <CardContent className="p-4 xl:p-6">
            ...
            <div className="flex flex-col lg:flex-row gap-3 xl:gap-4 items-center justify-between mb-4 xl:mb-6">
              ...
              <div className="flex flex-wrap gap-2 xl:gap-3 w-full lg:w-auto justify-center lg:justify-end">
                ...
                <Button
                  variant="outline"
                  onClick={exportToCSV}
                  className="w-32 xl:w-40 h-12 xl:h-14 border-2 border-slate-200 focus:border-blue-500 rounded-2xl bg-white/90 backdrop-blur-sm shadow-lg hover:border-blue-400 hover:bg-blue-50 font-bold transition-all duration-300"
                >
            ...
            {/* Modern Table - Lecturer Registration Style */}
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
              ...
              {/* Pagination - Lecturer Registration Style */}
