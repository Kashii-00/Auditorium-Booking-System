...
export default function LecturerView() {
  ...
  useEffect(() => {
    const fetchLecturer = async () => {
      ...
      setError(null)
      try {
        const data = await authRequest("get", getApiUrl(`/lecturer-registration/${id}`))
        setLecturer(data)
      } catch (err) {
  ...
  const handleDeleteLecturer = async () => {
    ...
    try {
      await authRequest("delete", getApiUrl(`/lecturer-registration/${id}`))
                ...
                <div>
                  ...
                  <div className="mt-1">
                    {lecturer.category ? (
                      <Badge 
                        variant="outline" 
                        className="bg-blue-50 text-blue-800 border-blue-300 font-semibold"
                      >
                        Category {lecturer.category}
                      </Badge>
                    ) : (
