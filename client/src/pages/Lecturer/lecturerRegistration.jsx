...
export default function LecturerManagementSystem() {
  ...
  const fetchLecturers = useCallback(async () => {
    try {
      ...
      const response = await authRequest("get", getApiUrl("/lecturer-registration"))
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (editingLecturer) {
          response = await authRequest("put", getApiUrl(`/lecturer-registration/${editingLecturer}`), formDataObj, {
          ...
          response = await authRequest("post", getApiUrl("/lecturer-registration"), formDataObj, {
            headers: { "Content-Type": "multipart/form-data" },
          })
  ...
  const fetchLecturerForEdit = useCallback(async (lecturerId) => {
    try {
      ...
      const lecturerData = await authRequest("get", getApiUrl(`/lecturer-registration/${lecturerId}`))
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      const coursesData = await authRequest("get", getApiUrl("/lecturer-registration/courses"))
  ...
  const handleSendPasswordReset = useCallback(
    async (lecturer) => {
      try {
        setLoading(true)
        await authRequest("post", getApiUrl("/lecturer-auth/forgot-password"), { email: lecturer.email })
  ...
  const handleDeleteLecturer = useCallback(
    async (lecturerId) => {
      ...
      if (confirmDeleteId === lecturerId) {
        ...
        try {
          setLoading(true)
          await authRequest("delete", getApiUrl(`/lecturer-registration/${lecturerId}`))
