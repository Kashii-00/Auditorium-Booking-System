...
const BatchAddLecturers = React.memo(({ onLecturersAdded }) => {
  ...
  const fetchBatch = useCallback(async () => {
    try {
              const batchData = await authRequest("get", getApiUrl(`/batches/${batchId}`))
      setBatch(batchData)
    } catch (err) {
  ...
  const fetchLecturers = useCallback(async () => {
    ...
    try {
              const allLecturers = await authRequest("get", getApiUrl("/lecturer-registration"))
              const assigned = await authRequest("get", getApiUrl(`/batches/${batchId}/lecturers`))
  ...
  const handleSelect = useCallback((id) => {
    setSelected((prev) => {
      ...
      // Initialize lecturer details when selected
      if (!prev.includes(id) && !lecturerDetails[id]) {
        setLecturerDetails(prevDetails => ({
          ...prevDetails,
          [id]: { module: "", hours_assigned: "" }
        }))
      }
  ...
  // Handle lecturer details change
  const handleLecturerDetailChange = useCallback((lecturerId, field, value) => {
  ...
  const toggleSelectAll = useCallback(() => {
    setSelected((prev) => {
      const newSelected = prev.length === filteredLecturers.length ? [] : filteredLecturers.map((lecturer) => lecturer.id)
      
      // Initialize details for newly selected lecturers
      const newDetails = { ...lecturerDetails }
      if (newSelected.length > 0) {
        newSelected.forEach(id => {
          if (!newDetails[id]) {
            newDetails[id] = { module: "", hours_assigned: "" }
          }
        })
        setLecturerDetails(newDetails)
      }
      
      return newSelected
    })
  }, [filteredLecturers, lecturerDetails])
  ...
  const handleAdd = useCallback(async () => {
    ...
    // Validate that all selected lecturers have module and hours assigned
    const missingDetails = selected.filter(id => {
      const details = lecturerDetails[id]
      return !details?.module?.trim() || !details?.hours_assigned?.trim()
    })

    if (missingDetails.length > 0) {
      setError("Please fill in module and hours for all selected lecturers.")
      return
    }
    ...
    try {
      ...
      // Prepare lecturer data with details
      const lecturerData = selected.map(id => ({
        lecturer_id: id,
        module: lecturerDetails[id].module.trim(),
        hours_assigned: parseInt(lecturerDetails[id].hours_assigned) || 0
      }))

              await authRequest("post", getApiUrl(`/batches/${batchId}/lecturers`), {
  ...
  }, [selected, batchId, onLecturersAdded, fetchLecturers, lecturerDetails])
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white">
                  {filteredLecturers.map((lecturer, index) => (
                    <tr 
                      key={lecturer.id} 
                      className={`${index !== filteredLecturers.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-colors ${
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Input
                          ...
                          value={lecturerDetails[lecturer.id]?.module || ""}
                          onChange={(e) => handleLecturerDetailChange(lecturer.id, "module", e.target.value)}
                          disabled={!selected.includes(lecturer.id)}
                          className={`w-40 text-sm ${!selected.includes(lecturer.id) ? 'bg-gray-100 text-gray-400' : 'bg-white'} ${
                            selected.includes(lecturer.id) && (!lecturerDetails[lecturer.id]?.module?.trim()) ? 'border-red-300' : 'border-gray-300'
                          }`}
                        />
                      </td>
                      <td className="px-6 py-4">
                        <Input
                          ...
                          min="0"
                          max="200"
                          value={lecturerDetails[lecturer.id]?.hours_assigned || ""}
                          onChange={(e) => handleLecturerDetailChange(lecturer.id, "hours_assigned", e.target.value)}
                          disabled={!selected.includes(lecturer.id)}
                          className={`w-20 text-sm ${!selected.includes(lecturer.id) ? 'bg-gray-100 text-gray-400' : 'bg-white'} ${
                            selected.includes(lecturer.id) && (!lecturerDetails[lecturer.id]?.hours_assigned?.trim()) ? 'border-red-300' : 'border-gray-300'
                          }`}
                        />
                      </td>
                    </tr>
