...
const BatchLecturers = React.memo(() => {
  ...
  const fetchBatchData = useCallback(async () => {
    try {
              const batchData = await authRequest("get", getApiUrl(`/batches/${id}`))
      setBatch(batchData)
    } catch (err) {
  ...
  const fetchBatchLecturers = useCallback(async () => {
    try {
      setLoading(true)
              const lecturersData = await authRequest("get", getApiUrl(`/batches/${id}/lecturers`))
  ...
  const handleDeleteLecturer = useCallback(async () => {
    ...
    try {
      ...
      const response = await authRequest(
        "delete",
        getApiUrl(`/batches/${id}/lecturers/${lecturerToDelete.id}`),
      )
  ...
  const handleEditModule = useCallback((lecturer) => {
    ...
    setHoursValue(lecturer.hours_assigned || "")
  ...
  const saveModuleChanges = useCallback(async () => {
    ...
    try {
      ...
      const response = await authRequest(
        "put",
        getApiUrl(`/batches/${id}/lecturers/${currentLecturer.id}`),
        { 
          module: moduleValue,
          hours_assigned: parseInt(hoursValue) || 0
        },
      )
      ...
      if (response.success) {
        setLecturers((prev) => prev.map((l) => 
          l.id === currentLecturer.id 
            ? { ...l, module: moduleValue, hours_assigned: parseInt(hoursValue) || 0 } 
            : l
        ))

        setSuccessMessage("Lecturer details updated successfully.")
        setTimeout(() => setSuccessMessage(""), 3000)
      ...
      setError("Failed to update lecturer details. Please try again later.")
    } finally {
      setLoading(false)
    }
  }, [currentLecturer, moduleValue, hoursValue, id])

  // Handle view lecturer details
  const handleViewLecturer = useCallback(
    (lecturerId) => {
    ...
    <div className="min-h-screen bg-gray-50">
                    ...
                    <tr key={lecturer.id} className={`${index !== lecturers.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-colors`}>
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                          {lecturer.hours_assigned ? `${lecturer.hours_assigned} hrs` : "Not set"}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Edit Lecturer Assignment</DialogTitle>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div>
              <p className="text-sm text-gray-500 mb-1">Lecturer</p>
              <p className="font-medium">{currentLecturer?.full_name || currentLecturer?.userName || "Unknown"}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Enter hours assigned"
                className="w-full"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={loading || !moduleValue.trim() || !hoursValue || hoursValue <= 0}
            >
