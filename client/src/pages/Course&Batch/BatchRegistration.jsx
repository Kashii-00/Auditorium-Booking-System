    ...
    state.batches.forEach(batch => {
      ...
      if (batch.batch_code && batch.batch_code.toLowerCase().includes(lowerTerm)) {
        suggestions.push({
          ...
          subtitle: batch.courseName || 'Batch',
          icon: 'batch'
        })
      }
      ...
      if (batch.courseName && batch.courseName.toLowerCase().includes(lowerTerm)) {
        suggestions.push({
          ...
          subtitle: batch.courseId || 'Course',
          icon: 'course'
        })
      }
      ...
      if (batch.courseId && batch.courseId.toLowerCase().includes(lowerTerm)) {
        suggestions.push({
          ...
          label: batch.courseId,
          subtitle: batch.courseName || 'Course ID',
          icon: 'id'
        })
      }
    })
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-200">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0 lg:space-x-6">
        ...
        {/* Enhanced Search with Fixed Positioning */}
        <div className="relative flex-1 max-w-md" ref={searchRef}>
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-sm bg-white"
            />
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <button
            onClick={onExport}
            className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium text-sm transition-colors flex items-center space-x-2 shadow-sm"
          >
...
const BatchRegistration = () => {
  ...
  const fetchBatches = useCallback(async () => {
    try {
      dispatch({ type: "SET_LOADING", payload: true })
              const batchesData = await authRequest("get", getApiUrl("/batches"))
  ...
  const fetchCourses = useCallback(async () => {
    try {
              const coursesData = await authRequest("get", getApiUrl("/CourseRegistrationRoute"))
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        if (isEditing) {
          await authRequest("put", getApiUrl(`/batches/${currentBatchId}`), formData)
          showToast("success", "Batch updated successfully!")
        } else {
          await authRequest("post", getApiUrl("/batches"), formData)
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      if (Array.isArray(batchToDelete)) {
        // Bulk delete
        await Promise.all(
                      batchToDelete.map((batch) => authRequest("delete", getApiUrl(`/batches/${batch.id}`))),
        )
        showToast("success", `${batchToDelete.length} batches deleted successfully!`)
        dispatch({ type: "CLEAR_SELECTION" })
      } else {
        // Single delete
                  await authRequest("delete", getApiUrl(`/batches/${batchToDelete.id}`))
