...
function CourseRegistrationForm({ onBack, onSuccess, editingCourse, isEditMode = false }) {
  ...
  // Handle tab navigation with optional validation
  const handleTabChange = useCallback((newStep) => {
  ...
  const checkCourseIdAvailability = async (id) => {
    ...
    try {
              const response = await authRequest("get", getApiUrl(`/CourseRegistrationRoute/check/${id}`))
      if (response.exists) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEditMode 
        ? getApiUrl(`/CourseRegistrationRoute/${editingCourse.id}`)
                    ...
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none z-10">
                        <div className="flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200">
                          <span className="text-emerald-700 font-bold text-xs">LKR</span>
                        </div>
                      </div>
            <Input
              id="fees"
              ...
              value={formData.fees}
                        onChange={handleInputChange}
                        className={`pl-20 mt-2 h-12 border-2 focus:border-blue-500 rounded-xl bg-white/90 backdrop-blur-sm shadow-lg ${
            ...
            <div>
              ...
              <Input
                ...
                value={formData.installment1}
                        onChange={handleInputChange}
                        className="pl-20 mt-2 h-12 border-2 border-slate-200 focus:border-blue-500 rounded-xl bg-white/90 backdrop-blur-sm shadow-lg"
                        placeholder="0.00"
                      />
                    </div>
            </div>
            ...
            <div className="mt-8">
              <div className="flex justify-between items-center">
                {formSections.map((section, index) => {
                  ...
                  // Allow navigation to any tab for better UX
                  const canNavigate = true

                  return (
                    <div key={index} className="flex flex-col items-center relative flex-1">
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center shadow-xl z-10 transition-all duration-300 cursor-pointer ${
                          isCompleted
                            ? "bg-gradient-to-br from-emerald-500 to-green-600 text-white hover:shadow-2xl hover:scale-105"
                            : isActive
                              ? "bg-gradient-to-br from-blue-500 via-indigo-600 to-purple-700 text-white ring-4 ring-blue-200"
                              : "bg-white text-slate-600 border-2 border-slate-300 hover:border-blue-400 hover:text-blue-600 hover:shadow-lg hover:scale-105"
                        }`}
                      ...
                      <p
                        className={`mt-2 font-bold text-sm transition-colors duration-300 text-center cursor-pointer ${
                          isActive ? "text-blue-700" : isCompleted ? "text-emerald-600" : "text-slate-600 hover:text-blue-600"
                        }`}
                        onClick={() => handleTabChange(index)}
                        title={`Go to ${section.title}`}
                      >
...
function CourseDashboard({ courses: initialCourses, setCourses, onRegisterClick, onEditCourse, isLoading, error, refreshCourses }) {
  ...
  const handleDeleteCourse = useCallback(async (courseId) => {
    ...
    if (confirmDeleteId === courseId) {
      try {
        await authRequest("delete", getApiUrl(`/CourseRegistrationRoute/${courseId}`))
    ...
    <>
      ...
      <Card className="border-0 shadow-lg bg-white">
        ...
        <CardContent className="p-0">
          ...
          <div className="flex justify-between items-center p-6 pb-1 bg-gradient-to-r from-slate-50 to-blue-50 border-b border-slate-200">
          ...
          <div>
            ...
            >
              {Array.isArray(course.medium) 
                ? course.medium.map(medium => typeof medium === 'string' ? medium.replace(/["\[\]]/g, '') : medium).join(", ")
                : typeof course.medium === 'string' 
                  ? course.medium.replace(/["\[\]]/g, '')
                  : course.medium}
            </Badge>
          </div>
        </div>
...
const CourseDetailsDialog = memo(({ course, isOpen, onClose, onEdit }) => {
  if (!course) return null

  const mediumArray = Array.isArray(course.medium) 
    ? course.medium.map(medium => typeof medium === 'string' ? medium.replace(/["\[\]]/g, '') : medium)
    : [course.medium].map(medium => typeof medium === 'string' ? medium.replace(/["\[\]]/g, '') : medium)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md bg-white border-0 shadow-2xl rounded-2xl mt-7">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
              ...
              <p className="text-sm font-bold text-green-800">{course.duration || "Not specified"}</p>
            </div>

            {/* Medium Card */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-center">
              <Globe className="h-5 w-5 text-blue-600 mx-auto mb-1" />
              <p className="text-xs text-blue-600 font-medium">Medium</p>
              <p className="text-sm font-bold text-blue-800">
                {mediumArray.length > 0 ? mediumArray.join(", ") : "N/A"}
              </p>
            </div>
          </div>
        ...
        <DialogFooter className="pt-4 gap-2">
          ...
          {onEdit && (
            <Button
              ...
              variant="outline"
              className="flex-1 border-orange-300 text-orange-600 hover:bg-orange-50 hover:border-orange-400 text-sm"
            >
...
function CourseManagementSystem() {
  ...
  const fetchCourses = async (showToast = false) => {
    try {
      setIsLoading(true)
      setError(null)
              const data = await authRequest("get", getApiUrl("/CourseRegistrationRoute"))
