...
const BatchDetailsPopup = React.memo(({ batchId, onClose }) => {
  ...
  const fetchBatchDetails = useCallback(async () => {
    ...
    try {
              const response = await authRequest("get", getApiUrl(`/batches/details/${batchId}`))
    ...
    try {
      const endpoint =
        deleteDialog.type === "student"
          ? getApiUrl(`/batches/${batchId}/students/${deleteDialog.id}`)
          : getApiUrl(`/batches/${batchId}/lecturers/${deleteDialog.id}`)
