    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const batchesData = await authRequest(
          "get",
          getApiUrl(`/batches?course_id=${courseId}&year=${year}`),
        )
          ...
          const detailsPromises = batchesData.map(async (batch) => {
            try {
              const [batchDetail, students, lecturers] = await Promise.all([
                authRequest("get", getApiUrl(`/batches/${batch.id}`)),
                                  authRequest("get", getApiUrl(`/batches/${batch.id}/students`)).catch(() => []),
                                  authRequest("get", getApiUrl(`/batches/${batch.id}/lecturers`)).catch(() => []),
              ])
