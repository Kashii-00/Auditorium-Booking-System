...
import { FaArrowLeft, FaPlus, FaUserGraduate, FaSearch, FaDownload, FaTrash, FaEye } from "react-icons/fa"
...
const BatchStudents = React.memo(() => {
  ...
  const generateSuggestions = useCallback((term) => {
    ...
    students.forEach(student => {
      // Search in student_code
      if (student.student_code && student.student_code.toLowerCase().includes(termLower)) {
        suggestions.push({
          type: 'Student ID',
      ...
      // Search in full_name
      if (student.full_name && student.full_name.toLowerCase().includes(termLower)) {
        suggestions.push({
          type: 'Name',
          value: student.full_name,
          label: `${student.full_name} (${student.student_code || 'No ID'})`,
          student
        })
      }
      // Search in email
      if (student.email && student.email.toLowerCase().includes(termLower)) {
        suggestions.push({
          type: 'Email',
          value: student.email,
          label: `${student.email} - ${student.full_name}`,
          student
        })
      }
      // Search in id_number
      if (student.id_number && student.id_number.toLowerCase().includes(termLower)) {
        suggestions.push({
          type: 'ID Number',
          value: student.id_number,
          label: `${student.id_number} - ${student.full_name}`,
          student
        })
      }
    })
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
              const studentsData = await authRequest("get", getApiUrl(`/batches/${batchId}/students`))
  ...
  const handleRemoveStudent = useCallback(
    async (studentId) => {
      ...
      try {
        await authRequest("delete", getApiUrl(`/batches/${batchId}/students/${studentId}`))
  ...
  const filteredStudents = useMemo(() => {
    try {
      ...
      return students.filter((student) => {
        ...
        // Search filter - now includes student_code
  ...
  const exportToCSV = useCallback(() => {
    try {
      const headers = ["Student Code", "ID", "Name", "Email", "Phone", "ID Number", "Status"]
          ...
          <div className="bg-white rounded-lg p-6 border border-gray-200">
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                {/* Capacity Warning */}
                {batch?.capacity && students.length >= batch.capacity * 0.9 && (
                  <div className="mt-2">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="flex flex-col md:flex-row gap-4 flex-1">
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <Input
                  placeholder="Search by Student ID, Name, Email or ID Number..."
                  value={searchTerm}
            ...
            <div className="flex items-center space-x-4">
              <div className="text-gray-600 text-sm">
                ...
                {batch?.capacity && (
                  <span className="ml-2 text-gray-500">
                    ({batch.capacity - students.length} spots remaining)
                  </span>
                )}
              </div>
              ...
              <Link to={`/batch/${batchId}/add-students`}>
                <Button 
                  ...
                  disabled={batch?.capacity && students.length >= batch.capacity}
                  title={batch?.capacity && students.length >= batch.capacity ? "Batch is at full capacity" : "Add students to this batch"}
                >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            disabled={!student.id}
                            className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 hover:text-blue-700 disabled:bg-gray-50 disabled:text-gray-400 transition-colors"
