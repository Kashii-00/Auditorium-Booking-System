...
import { getApiUrl, getBaseUrl } from '../../utils/apiUrl'
import {
  Loader2,
  Edit,
  Trash2,
...
export default function StudentView() {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        ...
        const studentData = await authRequest("get", getApiUrl(`/students/${id}`)) 
  ...
  const handleDeleteStudent = async () => {
    try {
      ...
      await authRequest("delete", getApiUrl(`/students/${id}`))
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start p-6 gap-6">
            ...
            <div className="flex-shrink-0">
              <Avatar className="h-24 w-24 border-4 border-white shadow-lg">
                {student.photo_path ? (
                  <AvatarImage src={`${getBaseUrl()}/${student.photo_path}`} />
                ) : (
              ...
              {student.nic_document_path || student.passport_document_path || student.photo_path ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {student.nic_document_path && (
                    <a
                      href={`${getBaseUrl()}/${student.nic_document_path}`}
                      target="_blank"
                  ...
                  {student.passport_document_path && (
                    <a
                      href={`${getBaseUrl()}/${student.passport_document_path}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-4 bg-gray-50 border border-gray-200 rounded-lg flex items-center gap-3 hover:bg-gray-100 transition-colors"
                    >
                  ...
                  {student.photo_path && (
                    <a
                      href={`${getBaseUrl()}/${student.photo_path}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-4 bg-gray-50 border border-gray-200 rounded-lg flex items-center gap-3 hover:bg-gray-100 transition-colors"
                    >
